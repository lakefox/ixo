...
module.exports = function () {
    ...
    this.block = function (index, data) {
    	if (this.bc.unSigned[index] == undefined) {
    		this.bc.unSigned[index] = [];
    	}
    	this.bc.unSigned[index].push(encodeURI(data.toString()));
    }
    this.sign = function (index, key) {
    	var prev = "";
    	for (var i = 0; i < this.bc.signed.length; i++) {
    		prev += hash(this.bc.signed[i]);
    	}
    	prev = hash(prev);
    	var block = this.bc.unSigned[index].join(" ");
    ...
    this.read = function (index, key) {
    	var prev = "";
    	for (var i = 0; i < index+1; i++) {
    		prev += hash(this.bc.signed[i]);
    	}
    	prev = hash(prev);
    	var block = this.bc.signed[index+1];
    	var blockSig = hash((hash(this.bc.signed[index]) + hash(key) + prev));
    	var dcBlock = unCipherHash(block, blockSig);
    	if (dcBlock.slice(0,1) != "x") {
    		return "Block Corrupt"
    	} else {
    		var sDCBlock = dcBlock.slice(1).split(" ");
	    	var pBlock = [];
	    	for (var i = 0; i < sDCBlock.length; i++) {
	    		pBlock.push(decodeURI(sDCBlock[i]));
	    	}
	    	return pBlock;
    	}
    }
    this.export = function () {
    	return this.bc.signed.slice(1).join(" ");
    }
    this.import = function (data) {
    	this.bc.signed = ["ixo"].concat(data.split(" "));
    }
}
